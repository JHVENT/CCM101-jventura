...
1. **Amazon EC2 (Elastic Compute Cloud)** — resizable virtual server instances for general-purpose computing.
2. **Amazon S3 (Simple Storage Service)** — scalable object storage for files, backups, and data lakes.
3. **Amazon RDS (Relational Database Service)** — managed relational databases (MySQL, PostgreSQL, SQL Server, etc.).
4. **AWS Lambda** — serverless, event-driven compute that runs code without provisioning servers.

## Three (3) Advantages
1. **Largest service catalog and market maturity** — the widest range of tools, third-party integrations, and community/documentation support of any provider.
2. **Global reach and reliability** — the largest number of regions/AZs among major providers, supporting high availability and data residency needs.
3. **Flexible, pay-as-you-go pricing** — no upfront hardware investment, with fine-grained cost control and reserved/spot pricing options for savings.

## Typical Enterprise Use Cases
- Hosting scalable web and mobile application backends.
- Data lakes and big data analytics (e.g., using S3, Redshift, EMR).
- Disaster recovery and backup across multiple regions.
- Machine learning and AI model training/deployment (e.g., SageMaker).
- Enterprise migration of legacy on-premises systems to the cloud.
